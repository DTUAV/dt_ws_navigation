...
- @b "/tf": odometry from the robot//机器人的里程计信息

//所有的坐标变换都会发到/tf这个话题，但是话题中frame_id 和child_frame_id是有区别和不同的，根据指定的frame_id 和child_frame_id获取坐标变换的数据
...
- @b "~throttle_scans": @b [int] throw away every nth laser scan//丢掉一些激光雷达数据
...
#include "nav_msgs/MapMetaData.h" //地图数据的消息格式
...
SlamGMapping::SlamGMapping():
    map_to_odom_(tf::Transform(tf::createQuaternionFromRPY( 0, 0, 0 ), tf::Point(0, 0, 0 ))), /*坐标变换包括旋转角度和起点*/
...
{
    std::string entropy_topic = "/entropy";
    private_nh_.param<std::string>("entropy_topic", entropy_topic, "/entropy");
    entropy_publisher_ = private_nh_.advertise<std_msgs::Float64>(entropy_topic, 1, true);

    std::string map_topic = "/map";
    private_nh_.param<std::string>("map_topic", map_topic, "/map");
    sst_ = node_.advertise<nav_msgs::OccupancyGrid>(map_topic, 1, true);

    std::string map_metadata_topic = "/map_metadata";
    private_nh_.param<std::string>("map_metadata_topic", map_metadata_topic, "/map_metadata");
    sstm_ = node_.advertise<nav_msgs::MapMetaData>(map_metadata_topic, 1, true);

    std::string dynamic_map_service_topic = "/dynamic_map_service_topic";
    private_nh_.param<std::string>("dynamic_map_service_topic", dynamic_map_service_topic, "/dynamic_map_service_topic");
    ss_ = node_.advertiseService(dynamic_map_service_topic, &SlamGMapping::mapCallback, this);
...
void SlamGMapping::publishLoop(double transform_publish_period)
{
    ...
    ros::Rate r(1.0 / transform_publish_period);//default = 20hz
    while(ros::ok())
    {
        publishTransform();
        r.sleep();
...
bool SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
    ...
    ident.setIdentity();
    ident.frame_id_ = laser_frame_;//源激光是在激光坐标系下(0,0,0)===激光设备在激光坐标系下是处于原点位置
    ident.stamp_ = scan.header.stamp;
    try
    {
        tf_.transformPose(base_frame_, ident, laser_pose);//得到激光相对机器人底盘坐标的位置//激光设备在底盘坐标系的位置//这个位置是固定不变的，因为激光设备安装时固定的
        /*
             void transformPose(const std::string &target_frame, const geometry_msgs::PoseStamped &stamped_in, geometry_msgs::PoseStamped &stamped_out) const
             target_frame就是你要把源pose转换成哪个frame上的pose。
             假如你的源pose的frame_id是"odom"，你想转到"map"上，那么target_frame写成“map”就可以了。stamped_in就是源pose，而stamped_out就是目标数据了，也就是转换完成的数据。
             需要注意的是，从参数上来看，转换时是不需要指定源frame_id的，这是因为它已经包含在了stamped_in中，换句话说，就是这个函数一个隐含的使用条件是，stamped_in中必须指明源pose属于哪个frame。
        */
    }
    catch(tf::TransformException e)
    {
...
//-----------------------------------------------------------------------------------------------------------------
    ...
    // 这里认为激光雷达数据的角度是对称的，且是从小到大递增的。其实不一定，也就是误差的来源
...
void SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    laser_count_++;                            //scan计数
    if ((laser_count_ % throttle_scans_) != 0) //应该是忽略前几次scan，默认是1，忽略第一次
        return;

    static ros::Time last_map_update(0,0);     //上一次地图更新的时间

    // We can't initialize the mapper until we've got the first scan
    if(!got_first_scan_)                       //判断是不是第一次接受scan，如果是
    {
        if(!initMapper(*scan))                 //初始化地图
            return;
        got_first_scan_ = true;                //改变标志位
    }
...
void SlamGMapping::publishTransform()
{
    ...
    /*
     sendTransform(tf::StampedTransform(transform, ros::Time::now(), "map", ”base_link“));
