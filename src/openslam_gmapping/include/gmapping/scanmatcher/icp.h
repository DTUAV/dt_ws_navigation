...
{
  //retval(x,y,theta) 即除了位置之外，还有角度，可以理解为机器人的位姿*/ 2D
  typedef typename PointPairContainer::const_iterator ContainerIterator;//为模板起别名
  PointPair mean = std::make_pair(Point(0.,0.), Point(0.,0.));//定义两个点云的中心
  int size=0;

  //求两坨点云的几何中心
  for (ContainerIterator it = container.begin(); it!=container.end(); it++)
  {
    mean.first = mean.first + it->first;//遍历累加
    mean.second = mean.second + it->second;//遍历累加
    size++;
  }
  mean.first = mean.first * (1. / size);//算出均值
  mean.second = mean.second*(1. / size);//算出均值
