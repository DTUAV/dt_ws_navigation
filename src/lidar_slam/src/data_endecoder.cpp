...
std::pair<std::string, bool> pack_data(const int &id, const int &scan_size, const int &pos_size, const int &num_point, const sensor_msgs::LaserScanConstPtr& scan_msg, const nav_msgs::OdometryConstPtr& odom_msg) {
  if (id == 0) {
    ...
    //S5: range数据
    for(const auto &val : scan_msg.get()->ranges) {
    ...
    //S6: 位姿数据(6)
    //x:
    tem = pose_to_string(odom_msg.get()->pose.pose.position.x, pos_size);
    if (tem.second)
      ret += tem.first;
    else
      return std::pair<std::string, bool>({}, false);
    ...
    //y:
    tem = pose_to_string(odom_msg.get()->pose.pose.position.y, pos_size);
    if (tem.second)
      ret += tem.first;
    else
      return std::pair<std::string, bool>({}, false);
    ...
    //z:
    tem = pose_to_string(odom_msg.get()->pose.pose.position.z, pos_size);
    if (tem.second)
      ret += tem.first;
    else
      return std::pair<std::string, bool>({}, false);
    ...
    //四元数 -->> 欧拉角
    tf::Quaternion quat;
    tf::quaternionMsgToTF(odom_msg.get()->pose.pose.orientation, quat);
    ...
    //S7: 时间戳
    ret += std::to_string(scan_msg.get()->header.stamp.toNSec());
...
bool uppack_data(const std::string &data, sensor_msgs::LaserScan::Ptr& scan_msg, nav_msgs::Odometry::Ptr& odom_msg) {
  ...
  if (id == 0) {
    ...
    scan_msg.get()->ranges.resize(num_point);

    for (int i = 0; i < num_point; ++i) {
      int start = 7 + i * scan_size;
      scan_msg.get()->ranges[i] = std::stof(data.substr(start, scan_size));
    }

    int odom_start = 7 + num_point * scan_size;
    //x
    odom_msg.get()->pose.pose.position.x = std::stof(data.substr(odom_start, point_size + 2));
    //y
    odom_start += point_size + 2;
    odom_msg.get()->pose.pose.position.y = std::stof(data.substr(odom_start, point_size + 2));
    //z
    odom_start += point_size + 2;
    odom_msg.get()->pose.pose.position.z = std::stof(data.substr(odom_start, point_size + 2));
    //yaw
    odom_start += point_size + 2;
    float yaw = std::stof(data.substr(odom_start, point_size + 2));
   ...
   odom_msg.get()->pose.pose.orientation.x = q.getX();
   odom_msg.get()->pose.pose.orientation.y = q.getY();
   odom_msg.get()->pose.pose.orientation.z = q.getZ();
   odom_msg.get()->pose.pose.orientation.w = q.getW();

   long long stamp = std::stoll(valid_data.substr(odom_start + point_size + 2));

   scan_msg.get()->header.stamp.fromNSec(stamp);
   odom_msg.get()->header.stamp.fromNSec(stamp);
  }
  else {
...
std::pair<std::string, bool> pose_to_string(const float &data, const int &bit) {
  ...
  //获取小数部分为2位
    int data_decimal = (int)((data - data_integer) * std::pow(10, 2));
...
float string_to_range(const std::string &data, const int &bit) {
  ...
  float ret = std::stof(integer) + std::stof(decimal) * std::pow(0.1, data.size() - bit);
  return ret;
}
